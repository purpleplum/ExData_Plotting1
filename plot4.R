...
##setup layout and marging
par(mfrow = c(2, 2), mar = c(4, 4, 2, 1), oma = c(0, 0, 2, 0))

##plot global_active_power vs time
with(sel_power,plot(Date_Time,Global_active_power,main = "",type="n",ylab = "Global Active Power", xlab = ""))
with(sel_power,lines(Date_Time,Global_active_power))

with(sel_power,plot(Date_Time,Voltage,main = "",type="n",ylab = "Voltage", xlab = "datetime"))
with(sel_power,lines(Date_Time,Voltage))
...
with(sel_power,plot(Date_Time,Global_reactive_power,main = "",type="n",ylab = "Global_reactive_power", xlab = "datetime"))
with(sel_power,lines(Date_Time,Global_reactive_power))
